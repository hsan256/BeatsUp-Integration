...
// import courses from 'src/redux/reducers/courses';

// ----------------------------------------------------------------------

export default function BookingNewestBooking({listSugg}) {
  ...
  return (
    <Box sx={{ py: 2 }}>
      ...
      <Slider ref={carouselRef} {...settings}>
        {listSugg?.map((sugg) => (
          <BookingItem key={sugg._id} sugg={sugg}/>
        ))}
      </Slider>
    </Box>
  );
    ...
    <Paper sx={{ mx: 1.5, borderRadius: 2, bgcolor: 'background.neutral' }}>
      ...
      <Stack spacing={2.5} sx={{ p: 3, pb: 2.5 }}>
        <Stack direction="row" alignItems="center" spacing={2}>
          <Avatar alt={sugg.title} src={user.imageUrl} />
          <div>
            <Typography variant="subtitle2">{sugg.title}</Typography>
      ...
      <Box sx={{ p: 1, position: 'relative' }}>
        ...
        <Image src={sugg.imageUrl} ratio="1/1" sx={{ borderRadius: 1.5 }} />
      </Box>
