...
const TABLE_HEAD = [
  ...
  { id: 'countInStock', label: 'Count InStock ', alignRight: false },
...
export default function EcommerceProductList() {
  ...
  const handleSelectAllClick = (checked) => {
    if (checked) {
      const selected = produitList.map((n) => n._id);
  ...
  const handleDeleteProduct = (produitId) => {
    const deleteProduiit = produitList.filter((produit) => produit._id !== produitId);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - produitList.length) : 0;
  const filteredProduits = applySortFilter(produitList, getComparator(order, orderBy), filterName);
              ...
              <Table>
                ...
                <TableBody>
                  {filteredProduits.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((produit) => {
                    const isItemSelected = selected.indexOf(produit._id) !== -1;
                    return (
                      <TableRow
                        hover
                        key={produit._id}
                      ...
                      >
                        <TableCell padding="checkbox">
                          <Checkbox checked={isItemSelected} onClick={() => handleClick(produit._id)} />
                        </TableCell>
                        ...
                        <TableCell style={{ minWidth: 160 }}>{fDate(produit.createdAt)}</TableCell>
                        <TableCell align="center">{(produit.status)}</TableCell>
                        <TableCell align="center">{(produit.countInStock)}</TableCell>
                        <TableCell align="right">{(produit.price)} DT</TableCell>
