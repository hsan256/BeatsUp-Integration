...
import { updateEvents } from '../../redux/actions/events';
// firebase
...
export default function CollectionNewForm() {
  ...
  const [EventData, setEventData] = useState({
    ...
    EventDes: single.EventDes,
    EventDate: single.EventDate,
    EventHost: single.EventHost,
    Eventphoto: single.Eventphoto,
  });
  ...
  const onSubmit = async () => {
    try {
      ...
      reset();
      handleClosePreview();
      dispatched(updateEvents(single._id, EventData));
      enqueueSnackbar('Event success!');
      navigate(PATH_DASHBOARD.event.posts);
    } catch (error) {
      console.error(error);
    }
  };
